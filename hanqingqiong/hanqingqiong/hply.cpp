...
HPLY::PlyFile* HPLY::ply_open_for_writing(
  char *filename,
  int nelems,
  char **elem_names,
  int file_type,
...
void HPLY::ply_describe_element(
  HPLY::PlyFile *plyfile,
  char *elem_name,
  int nelems,
  int nprops,
  HPLY::PlyProperty *prop_list
)
{
  int i;
  PlyElement *elem;
...
void HPLY::ply_describe_property(
  HPLY::PlyFile *plyfile,
  char *elem_name,
  HPLY::PlyProperty *prop
)
{
  PlyElement *elem;
...
void HPLY::ply_describe_other_properties(
  HPLY::PlyFile *plyfile,
  HPLY::PlyOtherProp *other,
  int offset
)
{
  int i;
...
void HPLY::ply_header_complete(HPLY::PlyFile *plyfile)
{
  int i,j;
  FILE *fp = plyfile->fp;
  PlyElement *elem;
...
void HPLY::ply_put_element_setup(HPLY::PlyFile *plyfile, char *elem_name)
{
  PlyElement *elem;
...
void HPLY::ply_put_element(HPLY::PlyFile *plyfile, void *elem_ptr)
{
  int j,k;
  FILE *fp = plyfile->fp;
  PlyElement *elem;
...
void HPLY::ply_put_comment(HPLY::PlyFile *plyfile, char *comment)
{
...
void HPLY::ply_put_obj_info(HPLY::PlyFile *plyfile, char *obj_info)
{
...
HPLY::PlyFile* HPLY::ply_read(FILE *fp, int *nelems, char ***elem_names)
{
  int i,j;
  PlyFile *plyfile;
  int nwords;
...
HPLY::PlyFile* HPLY::ply_open_for_reading(
  char *filename,
  int *nelems,
  char ***elem_names,
  int *file_type,
...
HPLY::PlyProperty** HPLY::ply_get_element_description(
  HPLY::PlyFile *plyfile,
  char *elem_name,
  int *nelems,
  int *nprops
...
void HPLY::ply_get_element_setup(
  HPLY::PlyFile *plyfile,
  char *elem_name,
  int nprops,
  PlyProperty *prop_list
...
void HPLY::ply_get_element(HPLY::PlyFile *plyfile, void *elem_ptr)
{
  if (plyfile->file_type == PLY_ASCII)
...
char** HPLY::ply_get_comments(HPLY::PlyFile *plyfile, int *num_comments)
{
...
char** HPLY::ply_get_obj_info(HPLY::PlyFile *plyfile, int *num_obj_info)
{
...
void HPLY::setup_other_props(HPLY::PlyElement *elem)
{
  int i;
  PlyProperty *prop;
  int size = 0;
...
HPLY::PlyOtherProp* HPLY::ply_get_other_properties(
  HPLY::PlyFile *plyfile,
  char *elem_name,
  int offset
)
...
HPLY::PlyOtherElems* HPLY::ply_get_other_element (
  HPLY::PlyFile *plyfile,
  char *elem_name,
  int elem_count
)
...
void HPLY::ply_describe_other_elements (
  HPLY::PlyFile *plyfile,
  HPLY::PlyOtherElems *other_elems
)
{
  int i;
  OtherElem *other;
...
void HPLY::ply_put_other_elements (HPLY::PlyFile *plyfile)
{
  int i,j;
  OtherElem *other;
...
void HPLY::ply_free_other_elements (HPLY::PlyOtherElems *other_elems)
{
...
void HPLY::ply_close(HPLY::PlyFile *plyfile)
{
  fclose (plyfile->fp);
...
void HPLY::ply_get_info(HPLY::PlyFile *ply, float *version, int *file_type)
{
  if (ply == NULL)
    return;
...
int HPLY::equal_strings(const char *s1, const char *s2)
{
...
HPLY::PlyElement* HPLY::find_element(HPLY::PlyFile *plyfile, char *element)
{
  int i;
...
HPLY::PlyProperty* HPLY::find_property(HPLY::PlyElement *elem, char *prop_name, int *index)
{
  int i;
...
void HPLY::ascii_get_element(HPLY::PlyFile *plyfile, char *elem_ptr)
{
  int j,k;
  PlyElement *elem;
  PlyProperty *prop;
...
void HPLY::binary_get_element(HPLY::PlyFile *plyfile, char *elem_ptr)
{
  int j,k;
  PlyElement *elem;
  PlyProperty *prop;
...
void HPLY::write_scalar_type (FILE *fp, int code)
{
...
void HPLY::swap_bytes(char *bytes, int num_bytes)
{
    int i;
    char temp;
...
void HPLY::get_native_binary_type()
{
union
{
      int  int_value;
...
char** HPLY::get_words(FILE *fp, int *nwords, char **orig_line)
{
#define BIG_STRING 4096
  static char str[BIG_STRING];
  static char str_copy[BIG_STRING];
  char **words;
  int max_words = 10;  //一行中的最多处理10个单词
  int num_words = 0;	//实际单词数
  char *ptr,*ptr2;
  char *result;
...
double HPLY::get_item_value(char *item, int type)
{
...
double HPLY::old_write_ascii_item(FILE *fp, char *item, int type)
{
  unsigned char *puchar;
  char *pchar;
  short int *pshort;
...
void HPLY::add_element (HPLY::PlyFile *plyfile, char **words)
{
  PlyElement *elem;
...
int HPLY::get_prop_type(const char *type_name)
{
  int i;
...
void HPLY::add_property (HPLY::PlyFile *plyfile, char **words)
{
  PlyProperty *prop;
  PlyElement *elem;
...
void HPLY::add_comment (HPLY::PlyFile *plyfile, char *line)
{
  int i;
...
void HPLY::add_obj_info (HPLY::PlyFile *plyfile, char *line)
{
  int i;
...
void HPLY::copy_property(HPLY::PlyProperty *dest, HPLY::PlyProperty *src)
{
...
char* HPLY::my_alloc(int size, int lnum, char *fname)
{
  char *ptr;
...
//HJM: 这不是原来的plyfile.c中的文件，是HJM所加
//从ply文件读到外部的文件
void HPLY::read_file(FILE* stdin_f, const char* out_v_file, const char* out_f_file)
{
  ...
  for (i = 0; i < nelems; i++) 
  {
    ...
    if (equal_strings ("vertex", elem_name))
	{
	  ...
	  for (j = 0; j < num_elems; j++) 
	  {
		...
		fprintf(fp_f_a,"%f %d  %d  %d\n", s, temp_f.verts[0], temp_f.verts[1], temp_f.verts[2]);
...
//HJM: 这不是原来的plyfile.c中的文件，是HJM所加
void HPLY::get_nverts_nfaces(const char* file, int* nvf) //
{
	FILE *in_f=fopen(file, "rb");
	if (!in_f)
    {
...
float HPLY::smallest(int* verts, HPLY::PlyFile* ply, FILE* fp_v) 
{

		int ver_length;
		float aa,bb,cc;//三个指标对应的顶点的横坐标
		int a,b,c;
		a=verts[0];	b=verts[1];	c=verts[2];
		 
		ver_length=3*sizeof(float)+sizeof(int);
		
		if( !fseek(fp_v, (a*ver_length), SEEK_SET) ) fread(&aa,sizeof(float),1,fp_v);//
		if( !fseek(fp_v, (b*ver_length), SEEK_SET) ) fread(&bb,sizeof(float),1,fp_v);//
		if( !fseek(fp_v, (c*ver_length), SEEK_SET) ) fread(&cc,sizeof(float),1,fp_v);//
		aa=Min3(aa,bb,cc);
		return aa;	//最小横坐标
}
